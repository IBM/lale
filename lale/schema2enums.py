...
def accumulateDiscoveredEnumsToPythonEnums(de:Optional[DiscoveredEnums], path:List[str], acc:Dict[str, enum.Enum])->None:
...
def addDictAsFields(obj:Any, d:Dict[str, Any])->None:
    if d is None:
        return
    for k, v in d.items():
        if k == "":
...
def addSchemaEnumsAsFields(obj:Any, schema:Schema)->None:
    enums = schemaToPythonEnums(schema)
    addDictAsFields(obj, enums)
