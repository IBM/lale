...
class SchemaRange(object):
    ...
    def fromSchemaForOptimizer(cls, schema:Any)->'SchemaRange':
        ...
        is_integer=schema.get('type', 'numberForOptimizer') == 'integer' or s.is_integer
    ...
    def to_schema_with_optimizer(cls, actual_range:'SchemaRange', optimizer_range:'SchemaRange')->Schema:
        ...
        if optimizer_range.minimum is not None:
            if (actual_range.minimum is None or
                actual_range.minimum < optimizer_range.minimum or
                  (actual_range.minimum == optimizer_range.minimum and 
                   optimizer_range.exclusive_minimum and not actual_range.exclusive_minimum)
                  ):
        ...
        if optimizer_range.maximum is not None:
            if (actual_range.maximum is None or
                actual_range.maximum > optimizer_range.maximum or
                  (actual_range.maximum == optimizer_range.maximum and 
                   optimizer_range.exclusive_maximum and not actual_range.exclusive_maximum)
                  ):
    ...
    def __iand__(self, other:'SchemaRange'):
        ...
        if other.minimum is not None:
            if self.minimum is None:
                ...
                self.exclusive_minimum = self.exclusive_minimum or other.exclusive_minimum
            elif self.minimum < other.minimum:
                self.minimum = other.minimum
                self.exclusive_minimum = other.exclusive_minimum
        ...
        if other.maximum is not None:
            if self.maximum is None:
                ...
                self.exclusive_maximum = self.exclusive_maximum or other.exclusive_maximum
            elif self.maximum > other.maximum:
                self.maximum = other.maximum
                self.exclusive_maximum = other.exclusive_maximum
    ...
    def diff(self, other:'SchemaRange')->Optional[bool]:
        ...
        if not other.is_integer or other.is_integer == self.is_integer:
            ...
            if (self.minimum is not None and other.maximum is not None and
                (other.maximum < self.minimum
                    or (other.maximum == self.minimum and
                    (self.exclusive_minimum or other.exclusive_maximum)))):
                return True
            # the exclusion is greater than the actual range
            if (self.maximum is not None and other.minimum is not None and
                (other.minimum > self.maximum
                    or (other.minimum == self.maximum and
                    (self.exclusive_maximum or other.exclusive_minimum)))):
                return True
            ...
            if other.minimum is None:
                ...
                if self.minimum is None or self.minimum < other.minimum or (self.minimum == other.minimum and (not self.exclusive_minimum or other.exclusive_minimum)):
                    if (other.maximum is None or
                            self.maximum is not None and (
                                other.maximum > self.maximum or (
                                (other.maximum == self.maximum and
                                    (not other.exclusive_maximum or self.exclusive_maximum)))
                                )
                            ):
                    ...
                    # self.minimum >= other.minimum
                    if other.maximum is None or self.maximum < other.maximum or (self.maximum == other.maximum and (not other.exclusive_maximum or self.exclusive_maximum)):
                        # nothing is possible
                        return None
                    else:
                        self.minimum = other.maximum
            ...
            if other.maximum is None:
                ...
                if self.maximum is None or self.maximum > other.maximum or (self.maximum == other.maximum and (not self.exclusive_maximum or other.exclusive_maximum)):
                    if (other.minimum is None or
                            self.minimum is not None and (
                                other.minimum < self.minimum or (
                                (other.minimum == self.minimum and
                                    (not other.exclusive_minimum or self.exclusive_minimum)))
                                )
                            ):
                        self.minimum = other.maximum
                        self.exclusive_minimum = not other.exclusive_maximum
                    ...
                    # self.maximum >= other.maximum
                    if (other.minimum is None or self.minimum > other.minimum or
        ...
        if lower.minimum is not None and upper.maximum is not None:
            ...
            if (lower.minimum == upper.maximum and
               (lower.exclusive_minimum or upper.exclusive_maximum)):
                return True
            if (is_integer and lower.exclusive_minimum and upper.exclusive_maximum 
