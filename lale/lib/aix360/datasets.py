...
def _get_compas_filepath(filename):
    directory = os.path.join(
        os.path.dirname(os.path.abspath(aix360.__file__)), "data", "raw", "compas"
    )
    return os.path.join(
        directory,
        filename,
...
def fetch_compas_violent_df(preprocess=False):
    ...
    if preprocess:
        ...
        dataset = aix360.datasets.StandardDataset(
            df=df,
...
def _fetch_meps_raw_df(panel, fiscal_year):
    ...
    filepath = os.path.join(
        os.path.dirname(os.path.abspath(aix360.__file__)),
        "data",
        "raw",
        "meps",
        filename,
...
def fetch_meps_panel19_fy2015_df(preprocess=False):
    ...
    """
    if preprocess:
        dataset = aix360.datasets.MEPSDataset19()
...
def fetch_meps_panel20_fy2015_df(preprocess=False):
    ...
    """
    if preprocess:
        dataset = aix360.datasets.MEPSDataset20()
        return _get_pandas_and_fairness_info_from_meps_dataset(dataset)
    else:
...
def fetch_meps_panel21_fy2016_df(preprocess=False):
    ...
    """
    if preprocess:
        dataset = aix360.datasets.MEPSDataset21()
        return _get_pandas_and_fairness_info_from_meps_dataset(dataset)
    else:
