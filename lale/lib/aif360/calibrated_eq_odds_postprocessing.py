...
class CalibratedEqOddsPostprocessingImpl(_BasePostprocessingImpl):
    ...
    def fit(self, X, y):
        return super(CalibratedEqOddsPostprocessingImpl, self).fit(X, y)

    def predict(self, X):
        return super(CalibratedEqOddsPostprocessingImpl, self).predict(X)
...
_hyperparams_schema = {
    ...
    {   'description':
        ...
        'required': (list(_postprocessing_base_hyperparams.keys())
                     + list(_additional_hyperparams.keys())),
        'relevantToOptimizer': ['cost_constraint'],
        'properties': {**_postprocessing_base_hyperparams,
                       **_additional_hyperparams}}]}
...
CalibratedEqOddsPostprocessing = lale.operators.make_operator(CalibratedEqOddsPostprocessingImpl, _combined_schemas)
