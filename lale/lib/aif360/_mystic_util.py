...
def obtain_solver_info(
    ...
    for pa in range(num_prot_attr):
        ...
        disadv_grp_adv_cls_ct = sum(mapped_osizes[x] for x in disadv_grp_adv_cls)
        disadv_grp_disadv_cls = [
            x for x in disadv_grp if int(x[-1]) not in favorable_labels
        ]
        disadv_grp_disadv_cls_ct = sum(mapped_osizes[x] for x in disadv_grp_disadv_cls)
        adv_grp_disadv_cls = [x for x in adv_grp if int(x[-1]) not in favorable_labels]
        adv_grp_disadv_cls_ct = sum(mapped_osizes[x] for x in adv_grp_disadv_cls)
        adv_grp_adv_cls = [x for x in adv_grp if int(x[-1]) in favorable_labels]
        adv_grp_adv_cls_ct = sum(mapped_osizes[x] for x in adv_grp_adv_cls)
        calc_di = (
