...
def obtain_solver_info(
    ...
    for pa in range(num_prot_attr):
        ...
        disadv_grp_disadv_cls_ct = sum(list(map(lambda x: mapped_osizes[x], disadv_grp_disadv_cls)))
        adv_grp_disadv_cls = list(filter(lambda x: int(x[-1]) not in favorable_labels, adv_grp))
        adv_grp_disadv_cls_ct = sum(list(map(lambda x: mapped_osizes[x], adv_grp_disadv_cls)))
        adv_grp_adv_cls = list(filter(lambda x: int(x[-1]) in favorable_labels, adv_grp))
