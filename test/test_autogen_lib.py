...
@pytest.mark.parametrize('name, Op',
    [(n, Op) for (n, Op) in kls if n in classifiers])
def test_classifier(name, Op):
    base_test(name, Op, load_iris)

@pytest.mark.parametrize('name, reason',
    [(n, r) for (n, r) in failed_classifiers])
...
@pytest.mark.parametrize('name, Op',
    [(n, Op) for (n, Op) in kls if n in multi])
...
@pytest.mark.parametrize('name, reason',
    [(n, r) for (n, r) in failed_regressors])
...
@pytest.mark.parametrize('name, reason',
    [(n, r) for (n, r) in failed_transformers])
