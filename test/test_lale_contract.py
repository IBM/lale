...
class TestLaleContract(unittest.TestCase):
    ...
    def setUp(cls) -> None:
        ...
        TestLaleContract.training_df = pd.read_csv(TestLaleContract.train_csv_path)

    def test_01_load_pickled_model(self):
        try:
            TestLaleContract.model = joblib.load(TestLaleContract.pickled_model_path)
            print(type(TestLaleContract.model))
            print(TestLaleContract.model)
        except Exception as e:
            assert False, f"Exception was thrown during model pickle: {e}"

    def test_02_predict_on_model(self):
        check_X = TestLaleContract.training_df.drop(['Risk'], axis=1).values
        x = check_X
        try:
            pred = TestLaleContract.model.predict(x)
    ...
    def test_03_print_pipeline(self):
        lale_pipeline = TestLaleContract.model
        TestLaleContract.pipeline_content = lale_pipeline.pretty_print()
        assert type(TestLaleContract.pipeline_content) == str
        assert len(TestLaleContract.pipeline_content) > 0
        assert "lale.wrap_imported_operators()" in TestLaleContract.pipeline_content

    def test_04_execute_pipeline(self):
        try:
            with open("pp_pipeline.py", "w") as pipeline_f:
                pipeline_f.write(TestLaleContract.pipeline_content)
            ...
            TestLaleContract.pp_pipeline = pipeline_module.pipeline
            assert(isinstance(TestLaleContract.pp_pipeline, lale.operators.TrainablePipeline))
        except Exception as e:
            assert False, f"{e}"
        finally:
            try:
    ...
    def test_05_train_pretty_print_pipeline(self):
        train_X = TestLaleContract.training_df.drop(['Risk'], axis=1).values
        train_y = TestLaleContract.training_df.Risk.values

        TestLaleContract.pp_pipeline.fit(train_X, train_y)

    def test_06_predict_on_pretty_print_pipeline(self):
        check_X = TestLaleContract.training_df.drop(['Risk'], axis=1).values
        x = check_X
        try:
            pred = TestLaleContract.pp_pipeline.predict(x)
    ...
    def test_07_convert_model_to_lale(self):
        ...
        try:
            lale_pipeline = TestLaleContract.model
            TestLaleContract.prefix_model = lale_pipeline.remove_last().freeze_trainable()
            assert(isinstance(TestLaleContract.prefix_model, lale.operators.TrainablePipeline))
        except Exception as e:
            assert False, f"Exception was thrown during model prediction: {e}"
    ...
    def test_08_refine_model_with_lale(self):
        ...
        try:
            print(f"prefix_model is of type: {type(TestLaleContract.prefix_model)}")
    ...
    def test_09_predict_refined_model(self):
        ...
        x = check_X
        try:
            pred = TestLaleContract.refined_model.predict(x)
            assert len(pred) == len(
